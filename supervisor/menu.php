...
<div id="content">
    ...
    <script>
        ...
        var myChart = new Chart(ctx, {
            type:'bar',
            data:{
                labels: ["Chocolate","Squash Grape","Lemon Dante","Squash Chocolate","Lemon Squash","Squash Strawberry"],
                data: [{
                    label:'',
                    data:[
                        <?php
                        $jumlah_Chocolate = mysqli_query($con, "SELECT * FROM orders where ='1'");
                        echo mysqli_num_rows($jumlah_Chocolate);?>,
                        <?php
                        $jumlah_Grape = mysqli_query($con, "SELECT * FROM orders where ='3'");
                        echo mysqli_num_rows($jumlah_Grape);?>,
                        <?php
                        $jumlah_Dante = mysqli_query($con, "SELECT * FROM orders where ='4'");
                        echo mysqli_num_rows($jumlah_Dante);?>,
                        <?php
                        $jumlah_Squash_Chocolate = mysqli_query($con, "SELECT * FROM orders where ='5'");
                        echo mysqli_num_rows($jumlah_Squash_Chocolate);?>,
                        <?php
                        $jumlah_Lemon_Squash = mysqli_query($con, "SELECT * FROM orders where ='6'");
                        echo mysqli_num_rows($jumlah_Lemon_Squash);?>,
                        <?php
                        $jumlah_Squash_Strawberry = mysqli_query($con, "SELECT * FROM orders where ='7'");
                        echo mysqli_num_rows($jumlah_Squash_Strawberry);?>,
                    ],
                    backgroundColor:[
                        'rgba(255, 99, 132, 0.2)',
                        'rgba(54, 162, 235, 0.2)',
